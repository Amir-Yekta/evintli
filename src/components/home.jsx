...
export default function HomePage() {
  ...
  function handleNavigation(category) {
    ...
    const updatedListing = listingLists.filter(
      (listing) => listing.category.toLocaleLowerCase() === category.toLocaleLowerCase()
    );
  ...
  function handleKeyDown(event) {
    if (event.key === 'Enter') {
      ...
      const updatedListingTitle = listingLists.filter(
        (listing) => listing.title.toLowerCase().includes(query.toLowerCase())
      );

      const updatedListingCompany = listingLists.filter(
        (listing) => listing.company.toLowerCase().includes(query.toLowerCase())
      );

      const updatedListingCategory = listingLists.filter(
        (listing) => listing.category.toLowerCase().includes(query.toLowerCase())
      );
